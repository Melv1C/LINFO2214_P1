...
int main(int argc, char **argv) {
    ...
    INFO("Argument du client : size: %d, rate: %d, time: %d, ip: %s, port: %d", size, rate, run_time, ip, port);
...
void* rcv(void* r) {
    ...
    //Send file id
    uint32_t fileindex = rand() % n_files;
    DEBUG("REQUEST OF FILE NBRE %d",fileindex);
    ...
    if(error!=0){
        ...
        DEBUG("SERVER RESPOND FOR REQUEST OF FILE %d",fileindex);
    }
